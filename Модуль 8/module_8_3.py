# target = """Цель: освоить навык создания пользовательских исключений и использовать его в решении задачи.
...
#     Выбрасывает исключение IncorrectCarNumbers с сообщением 'Неверная длина номера', переданная
...
class Car:
	...
	def set_vin(self, vin_int):
		if self.__is_valid_vin(vin_int):
			self.__vin = vin_int

	def set_numbers(self, numbers_string):
		if self.__is_valid_numbers(numbers_string):
			self.__numbers = numbers_string
